...
SELECT ICPCSynonyms.ConceptId AS ConceptId, ICPCSynonyms.Synonym AS Label, "2" AS Type 
FROM ICPCSynonyms 
...
# change file name to the relevant HI refset filename & location
LOAD DATA LOCAL INFILE '/Users/rory/Dev/SCTUtils/RFE_refset.txt' INTO TABLE ICPCRefset IGNORE 1 LINES;
...
CREATE INDEX id63 ON ICPCSynonyms(DescId);
CREATE INDEX id74 ON ICPCSynonyms(ConceptId);

# takes 70 minutes to run
INSERT INTO SCT_Concepts (ConceptId, Label, Type)
SELECT ICPCSynonyms.ConceptId AS ConceptId, ICPCSynonyms.Synonym AS Label, "1" AS Type 
FROM ICPCSynonyms 
INNER JOIN curr_langrefset_f ON curr_langrefset_f.ReferencedComponentId = ICPCSynonyms.DescId
WHERE curr_langrefset_f.AcceptabilityId = "900000000000548007" AND curr_langrefset_f.Active = '1' AND 
curr_langrefset_f.refsetId = "900000000000509007" AND ICPCSynonyms.Type = "900000000000013009";

CREATE INDEX id5 ON SCT_Concepts(ConceptId);
